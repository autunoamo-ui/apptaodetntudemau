...
import { DerivativeExamGenerator } from './components/DerivativeExamGenerator';
import {
  Sparkles,
  Upload,
  FileText,
...
const App: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'standard' | 'derivative'>('standard');
  ...
  return (
    <div className="min-h-screen flex flex-col bg-navy-950 font-sans selection:bg-gold-400 selection:text-navy-900">
                ...
                <div className="absolute right-0 top-full mt-2 w-72 bg-navy-900 border gold-border rounded-2xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
                  ...
                  {AVAILABLE_MODELS.map(model => (
                    <button
                      ...
                      className={`w-full text-left p-3 hover:bg-white/5 transition-colors flex items-start gap-3 ${preferredModel === model.id ? 'bg-gold-400/10' : ''
                        }`}
                    >
                      <div className={`w-3 h-3 rounded-full mt-1 shrink-0 border-2 ${preferredModel === model.id ? 'bg-gold-400 border-gold-400' : 'border-slate-600'
                        }`} />
      ...
      {/* 1.5. TAB NAVIGATION */}
      <nav className="bg-navy-900 border-b gold-border px-4 md:px-8 py-2 md:py-0 flex items-center gap-1 overflow-x-auto no-scrollbar">
        <button
          onClick={() => setActiveTab('standard')}
          className={`flex items-center gap-2 px-4 py-3 md:py-4 text-[10px] font-black uppercase tracking-[0.2em] transition-all relative shrink-0 ${activeTab === 'standard' ? 'text-gold-400' : 'text-slate-500 hover:text-slate-300'
            }`}
        >
          <LayoutGrid size={14} />
          Tạo đề chuẩn (2025)
          {activeTab === 'standard' && <div className="absolute bottom-0 left-0 right-0 h-1 bg-gold-400 rounded-t-full shadow-[0_0_10px_#fbbf24]" />}
        </button>

        <button
          onClick={() => setActiveTab('derivative')}
          className={`flex items-center gap-2 px-4 py-3 md:py-4 text-[10px] font-black uppercase tracking-[0.2em] transition-all relative shrink-0 ${activeTab === 'derivative' ? 'text-gold-400' : 'text-slate-500 hover:text-slate-300'
            }`}
        >
          <FileSearch size={14} />
          Tải đề mẫu & Ngữ liệu
          {activeTab === 'derivative' && <div className="absolute bottom-0 left-0 right-0 h-1 bg-gold-400 rounded-t-full shadow-[0_0_10px_#fbbf24]" />}
        </button>
      </nav>
      ...
      <div className="flex-1 overflow-y-auto">
        {activeTab === 'standard' ? (
          <>
            ...
            {/* 3. EXAM CONTENT */}
            <main className="p-4 md:p-12 relative">
